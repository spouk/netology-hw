...
тестирование ответа с поднятного контейнера с приложением через gitlab с использованием curl  не дает ожидаемого ответа 
я не могу получить ответ, подключение по порту проходит, ответа нет, из консоли запрос обрабатывается корректно (см.скрин выше)
...
- [`файл gitlab-ci.yml`](files/.gitlab-ci.yml)
