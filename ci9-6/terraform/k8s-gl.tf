...
locals {
  folder_id             = var.vm_web_folder_id       # Set your cloud folder ID.
  k8s_version           = var.k8s_version            # Set the Kubernetes version.
  zone_a_v4_cidr_blocks = var.k8s_cidr               # Set the CIDR block for subnet.
  sa_name               = "admin"                    # Set the service account name.
  registry_name         = "Container Registry1"       # Set the name of Yandex Container Registry.
}
...
resource "yandex_vpc_security_group_rule" "v5050" {
...
#resource "yandex_container_registry" "container" {
